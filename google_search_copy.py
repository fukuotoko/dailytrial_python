...
def get_info(driver, keyword):
    ...
    urls = driver.find_elements_by_css_selector('div.Z26q7c.UK95Uc.jGGQ5e > div > div > a > h3')
    if urls:
        for url in urls:
            items['url'].append(urls.get_attribute('href').strip())

    # seleniumによるtitleの取得
    titles = driver.find_elements_by_css_selector('div.Z26q7c.UK95Uc.jGGQ5e > div > div > a > h3')
    if titles:
        for title in titles:
            items['title'].append(title.text.strip())

    # seleniumによるdescription（説明文）の取得
    descriptions = driver.find_elements_by_css_selector('div.Z26q7c.UK95Uc > div.VwiC3b.yXK7lf.MUxGbd.yDYNvb.lyLwlc.lEBKkf > span')
    if descriptions:
...
def spread_out(items):
    ...
    try :
        ...
        for description in items['description']:
            worksheet.update_cell(3, column, description)
            column += 1
